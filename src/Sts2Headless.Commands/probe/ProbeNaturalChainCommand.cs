...
// (`just runner::probe::natural-chain`) and writes to documentation/research/.
internal static class ProbeNaturalChainCommand
{
    ...
    private static string FormatCatalog(Sts2Bindings.CatalogResult c, ulong seed)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"Generated by `just runner::probe::natural-chain` (seed={seed}). Walks `PlayerCmd.EndTurn` with Player.NetId = LocalContext.NetId = 1uL and no manual side-switch fallback. Each gap below is a `MissingMethodException` / `NullReferenceException` / etc. that the natural chain raised — the Phase-2 punch list.");
        sb.AppendLine();
